...
class DanbooruViewer:
    ...
    def setup_gui(self):
        ...
        # Bind hover effects
        self.random_btn.bind("<Enter>", lambda e: self.on_button_hover(self.random_btn, True))
        self.random_btn.bind("<Leave>", lambda e: self.on_button_hover(self.random_btn, False))
    ...
    def fetch_image_thread(self):
        try:
            ...
            if not self.posts_cache:
                ...
                if not self.posts_cache:
                    self.root.after(0, lambda: self.show_error("No images found with the specified tag."))
                    return
            ...
            print_json(json_str)
            #print(json.dumps(self.current_post, indent=4))
            print(f"{'\033[35m'}************************************************************************************************************")
    ...
    def display_image(self):
        try:
            ...
            self.download_btn.bind("<Enter>", lambda e: self.on_button_hover(self.download_btn, True))
            self.download_btn.bind("<Leave>", lambda e: self.on_button_hover(self.download_btn, False))
            ...
            # Get and format tags
            tag_string = self.current_post.get('tag_string_general', '') or self.current_post.get('tag_string', '')
    ...
    def download_image(self):
        ...
        try:
            ...
            if not filename:
                filename = f"danbooru_image_{self.current_post.get('id', 'unknown')}.jpg"
